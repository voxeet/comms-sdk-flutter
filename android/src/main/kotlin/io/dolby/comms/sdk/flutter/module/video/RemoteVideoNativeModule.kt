...
class RemoteVideoNativeModule(private val scope: CoroutineScope) : NativeModule {
    ...
    override fun onAttached(flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
        channel = MethodChannel(flutterPluginBinding.binaryMessenger, "dolbyio_remote_video_channel")
    ...
    private fun start(call: MethodCall, result: Result) = scope.launch(
        ...
        onSuccess = {
            ParticipantMapper
                ...
                ?.let { VoxeetSDK.video().remote.start(it).await() }
    ...
    private fun stop(call: MethodCall, result: Result) = scope.launch(
        ...
        onSuccess = {
            ParticipantMapper
                .fromMap(call.arguments as? Map<String, Any?>)
                ?.let { VoxeetSDK.video().remote.stop(it).await() }
                ?.let { require(it) { "Could not stop video" } }
                ?.also { result.success(null) }
                ?: throw IllegalStateException("Could not find participant")
        }
