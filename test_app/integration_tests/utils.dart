...
const voxeetSDKAsertsMethodChannel =
    MethodChannel('IntegrationTesting.VoxeetSDKAsserts');
const sessionServiceAssertsMethodChannel =
    MethodChannel('IntegrationTesting.SessionServiceAsserts');
const conferenceServiceAssertsMethodChannel =
    MethodChannel('IntegrationTesting.ConferenceServiceAsserts');
const recordingServiceAssertsMethodChannel =
    MethodChannel('IntegrationTesting.RecordingServiceAsserts');
...
Future<void> expectNative(
    ...
    required dynamic expected}) async {
  _handleNativeResult(await methodChannel.invokeMethod(assertLabel, expected),
      "Expect native", assertLabel);
...
Future<void> runNative(
    ...
    dynamic args}) async {
  _handleNativeResult(
      await methodChannel.invokeMethod(label, args), "Run native", label);
  return Future<void>.value();
}

Future<void> resetSDK() async {
  return await runNative(
      methodChannel: voxeetSDKAsertsMethodChannel, label: "resetVoxeetSDK");
}
